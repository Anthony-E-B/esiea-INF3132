    ...
    protected Monster parseBlock(Map<String, String> blockData){
        ...
        switch (type) {
            case WATER:
                ...
                return new WaterMonster(name, getStat(hpMin, hpMax), getStat(attackMin, attackMax), getStat(defenseMin, defenseMax), getStat(speedMin, speedMax), attacks, effect1, effect2); 
            case ELECTRIC:
                effect1 = Double.parseDouble(blockData.get("Paralysis"));
                return new ElectricMonster(name, getStat(hpMin, hpMax), getStat(attackMin, attackMax), getStat(defenseMin, defenseMax), getStat(speedMin, speedMax), attacks, effect1); 
            case FIRE:
                return new FireMonster(name, getStat(hpMin, hpMax), getStat(attackMin, attackMax), getStat(defenseMin, defenseMax), getStat(speedMin, speedMax), attacks); 
            case GROUND:
                return new GroundMonster(name, getStat(hpMin, hpMax), getStat(attackMin, attackMax), getStat(defenseMin, defenseMax), getStat(speedMin, speedMax), attacks); 
            case INSECT:
                return new InsectMonster(name, getStat(hpMin, hpMax), getStat(attackMin, attackMax), getStat(defenseMin, defenseMax), getStat(speedMin, speedMax), attacks); 
            case GRASS:
                return new GrassMonster(name, getStat(hpMin, hpMax), getStat(attackMin, attackMax), getStat(defenseMin, defenseMax), getStat(speedMin, speedMax), attacks); 
            case NORMAL: 
                return new NormalMonster(name, getStat(hpMin, hpMax), getStat(attackMin, attackMax), getStat(defenseMin, defenseMax), getStat(speedMin, speedMax), attacks);
